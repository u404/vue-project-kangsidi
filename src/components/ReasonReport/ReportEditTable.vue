      ...
      <div class="base-body-table-box" ref="bodyBox" @scroll.passive="onScroll">
        <table class="base-table base-body-table" :class="tableClass" ref="bodyTable">
          ...
          <template v-for="(item1, index1) in list">
            <tr class="level-1" :class="{active: active[index1]}" :key="'table-tr-'+index1" @click="rowClick(item1, 1)">
              <td>
                <i class="toggle-btn" @click.stop="toggleActive(index1)" v-if="item1.list && item1.list.length>0"></i>
                {{item1.baobiao_name}}
              </td>
              ...
              <td class="base-overflow" v-if="isPreview || +item1.is_report===0" @click.stop v-tooltip="{content: item1.report, classes: 'content-tooltip' , placement: 'left'}">{{item1.report}}</td>
              <td class="reason-wrap" v-else @click.stop v-tooltip="{content: item1.report, classes: 'content-tooltip' , placement: 'left'}">
                <input class="reason-input" :class="{warning: true}" type="text" v-model="item1.report" @focus="cacheItem(item1)" @change="updateReportItemReason(item1.id, item1.report)" @click.stop />
              </td>
            </tr>
            <template v-for="(item2,index2) in item1.list">
              <tr class="level-2" :class="{active: active[index1] && active[index1][index2]}" v-show="active[index1]" :key="'table-tr-' + index1 + '-'+index2" @click="rowClick(item2, 2)">
                <td>
                  <i class="toggle-btn" @click.stop="toggleActive(index1, index2)" v-if="item2.list && item2.list.length>0"></i>
                  {{item2.baobiao_name}}
                </td>
                ...
                <td class="base-overflow" v-if="isPreview || +item2.is_report===0" @click.stop v-tooltip="{content: item2.report, classes: 'content-tooltip' , placement: 'left'}">{{item2.report}}</td>
                <td class="reason-wrap" v-else @click.stop v-tooltip="{content: item2.report, classes: 'content-tooltip' , placement: 'left'}">
                  <input class="reason-input" :class="{warning: true}" type="text" v-model="item2.report" @focus="cacheItem(item2)" @change="updateReportItemReason(item2.id, item2.report)" @click.stop />
                </td>
              </tr>
              <template v-for="(item3,index3) in item2.list">
                <tr class="level-3" v-show="active[index1] && active[index1][index2]" :key="'table-tr-' + index1 + '-' + index2 + '-' + index3" @click="rowClick(item3, 3)">
...
export default {
  ...
  data () {
    return {
      ...
      editReasonDialogDisplay: false,
      editItem: null,
      reportWorkParam: 'yuanyinreport'
    }
  },
  computed: {
    unexplained () {
  ...
  methods: {
    ...
    updateReportItemReason (id, text) {
      this.$services.manage
        ...
        .catch(err => {
          ...
          this.$dialog.alert({
            type: 'error',
            msg: (err && err.msg) || '自动保存失败，该条原因已被还原'
          })
        })
    },
    ...
    loadDataList (callback) {
      ...
      let paramsExt = {}
      paramsExt[this.viewTypeConfig.reportEditOptionKey] = this.option
      this.$services.manage
        .getProblemDataList({
          work: this.workParam,
          params: {
    ...
    rowClick (itemData, level) {
      this.$emit('rowclick', itemData, level)
    },

    editReason (item) {
      this.editItem = item
      this.editReasonDialogDisplay = true
    }
  }
}
</script>
