<template>
  <div class="report-edit-wrap">
    <div class="report-total-tips">待润色
            ...
            <template v-for="(item2,index2) in item1.list">
              ...
              <template v-for="(item3,index3) in item2.list">
                <tr class="level-3" v-show="active[index1] && active[index1][index2]" :key="'table-tr-' + index1 + '-' + index2 + '-' + index3" @click="rowClick(item3)">
                  ...
                  <td class="base-overflow" v-if="isPreview || +item3.is_report===0" @click.stop v-tooltip="{content: item3.report, classes: 'content-tooltip' , placement: 'left'}">{{item3.report}}</td>
                  <td class="reason-wrap" v-else @click.stop v-tooltip="{content: item3.report, classes: 'content-tooltip' , placement: 'left'}">
