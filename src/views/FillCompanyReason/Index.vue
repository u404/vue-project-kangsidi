    ...
    <div class="base-main-panel">
      <div class="control-bar">
        <div class="time-filter-box flex">
          ...
          <span class="tips">提示：请同时选择年、月份后进行筛选</span>
        </div>
        <div class="btn-box flex">
          <button class="base-btn base-line-btn query-btn" @click="query">查阅</button>
        </div>
      </div>
      <filter-table ref="filterTable" :year="selectedYear" :month="selectedMonth"></filter-table>
    </div>
  </div>
</template>
...
import FilterTable from '@/components/FillCompanyReason/FilterTable'
export default {
  data () {
    return {
      selectedYear: '',
  ...
  methods: {
    setCache () {
      utils.localStorage.set('fillCompanyReasonData', {
        selectedYear: this.selectedYear,
        selectedMonth: this.selectedMonth
      })
    },
    getCache () {
      let data = utils.localStorage.get('fillCompanyReasonData')
      if (data) {
